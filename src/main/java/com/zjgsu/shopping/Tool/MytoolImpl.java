...
public class MytoolImpl implements Mytool {
    ...
    public BuyerHistoryList toBuyerHistoryList(List<BuyerHistory> li) {
      ...
      for(BuyerHistory item :li ){
          list.AddItem(item.getHistoryId(),item.getGoodId(),item.getBuyerId(),item.getSellerId(),goodMapper.getGoodInfo(item.getGoodId()).getGoodName(),item.getDate(),item.getNumber(),item.getBuyerName());
      }
      return list;
    }
